...
//////////memory：内存
//
////对应头文件：#include<string.h> 或 #include<memory.h>
// 
//目录：
//memcpy()
//memmove()
...
////////几个内存造作函数的共同点：
////////1.指针参数都是void*类型，因为直接对内存进行操作，所以可以对任意类型使用，故用void*
////////2.参数都要上传 size_t count ，也就是想要操作的空间，以字节为单位
////////3.由于指针类型都是void*，且count以字节为单位，使用函数运作时要将指针强转为(char*)类型
////////4.以上三点保证了内存操作函数对内存进行修改时，都是按照内存原本的顺序，一字节一字节的修改【无论原本是什么类型，现在都是以字节为单位】
////////5.其他特点可以参考字符串函数
...
//	while (count--)
//	{
//      ////这里强制类型转换处记得加括号，单目操作符，优先级相同时，结合性从右→左，故加括号防止强制类型转换前先++
//		*((char*)dest)++ = *((char*)src)++; 
//	}
//	return ret;
//}
...
//	void* ret = dest;
//
//	if ((char*)dest - (char*)src > 0)//若dest地址大于src，则从结尾开始往倒着拷贝内存
//	{
//		((char*)dest) += count - 1;
//		((char*)src) += count - 1;
//		while (count--)
...
//////本函数按照内存的大端小端原本的顺序，从低地址到高地址一一进行比较
//// 
//////  返回值  两块缓冲区第一个不同字节的关系
//////  > 0     buf1 >  buf2
//////    0     buf1 == buf2
...
////
////
//////声明：void *memset( void *dest, int c, size_t count );//count为拷贝的空间大小，单位字节
////
////
//////memset函数将从dest开始，把后面count个字节的内存区域设置成想要的内容
//
//
//int main()
//{
...
//	//由于memset是一个字节一个字节进行拷贝的，所以拷贝结果为 0x 01 01 01 01 01 01 01 01 01 01 00 00 00 00 00 
//	//0x01010101 的值为 16843009 ；0x01010000,小端字节序存储，其值为0x00000101,为257
//
//	return 0;
//}
